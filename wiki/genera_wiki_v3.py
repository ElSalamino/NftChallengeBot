    ...
    for name in sorted((x for x in names if isinstance(x, str) and x.strip()), key=str.lower):
        ...
        if is_equipment:
            forging = {
                ...
                "lvmax": "Nel dungeon trova la stanza Fabbro e scegli Approcciala. Devi possedere almeno uno dei quattro scaglioni del medaglione celeste e avere un LVX nello zaino: il Fabbro consuma 1 LVX e crea 1 LVMAX. Lo scaglione viene controllato ma non consumato. LVMAX vale il 250% delle statistiche base.",
            }
        rows.append({
            "name": name,
            "types": types,
...
SET_DETAIL_JS = r'''function setDetail(name){const s=find(D.sets,name);if(!s)return missing(name);return head(s.name,'Composizione, approcci ed effetti del set.','Set')+`<div class="detail"><div>${sectionTitle('Come si compone')}${chipLinks('item',s.components)}${sectionTitle('Effetto')}<div class="effect human">${esc(s.human||'Descrizione narrativa non presente.')}</div><div class="effect">${esc(s.technical||'Descrizione tecnica non presente.')}</div>${Object.values(s.bonus||{}).some(Number)?sectionTitle('Bonus grezzi')+stats(s.bonus):''}${s.approaches?.length?sectionTitle('Approcci disponibili')+chipLinks('approach',s.approaches):''}</div><aside class="sidebox">${img('sets',s.name,'🧩')}</aside></div>`}'''
